...
/**
 * Sidebar component - Navigation sidebar with user profile and menu items
 * Features: Responsive design, animations, click-outside to close
 */
const Sidebar = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (isOpen && window.innerWidth < 768 && !event.target.closest('.sidebar-container')) {
        onClose();
      }
    };
  ...
  return (
    <motion.div
      className="sidebar-container fixed top-16 left-0 h-full w-64 bg-white text-black z-50 md:z-30 shadow-lg"
      initial="closed"
